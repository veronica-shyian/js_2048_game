...
class Game {
    ...
    appearRandomTile() {
        ...
        this.board.forEach((row, rowIndex)=>{
            row.forEach((cell, cellIndex)=>{
                if (cell === 0) emptyCellsCoord.push([
                    rowIndex,
                    cellIndex
                ]);
            });
    ...
    shiftAndMerge(row) {
        ...
        const shiftRow = [];
        while(nonEmptyCells.length)if (nonEmptyCells[0] === nonEmptyCells[1]) {
        ...
        while(shiftRow.length < 4)shiftRow.push(0);
        return shiftRow;
    }
    moveLeft() {
        let hasMoved = false;
    ...
    hasAvailableMove() {
        for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
            ...
            if (col < 3 && cell === this.board[row][col + 1] || row < 3 && cell === this.board[row + 1][col]) return true;
        }
        return false;
    }
    updateStatus() {
        if (this.board.some((row)=>row.includes(2048))) this.status = "win";
        else if (!this.hasAvailableMove()) this.status = "lose";
